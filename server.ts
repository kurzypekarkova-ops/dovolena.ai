...
`;
  ...
  try {
    ...
    console.error("Gemini API Error in /api/itinerary:", error);
    res.status(500).json({ error: "Chyba při komunikaci s AI: " + error.message });
  }
});
...
`;
  ...
  try {
    ...
    console.error("Gemini API Error in /api/checklist:", error);
    res.status(500).json({ error: "Chyba při komunikaci s AI: " + error.message });
  }
});
...
`;
  ...
  try {
    ...
    console.error("Gemini API Error in /api/roast:", error);
    res.status(500).json({ error: "Chyba při hodnocení balení: " + error.message });
  }
});
...
// Vite configuration & static asset routing
async function startServer() {
  if (process.env.VERCEL) {
    // Under Vercel, the express app only serves API routes.
    // Static assets are handled by Vercel natively via vercel.json routes.
    return;
  }
